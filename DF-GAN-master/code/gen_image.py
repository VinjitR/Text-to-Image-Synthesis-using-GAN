...
def sampling(text_encoder, netG, dataloader,device):
    ...
    netG.load_state_dict(torch.load('models/%s/netG.pth'%(cfg.CONFIG_NAME), map_location=torch.device('cuda:0')))
    netG.eval()
...
def gen_example(input_text,output_dir,wordtoix,netG,text_encoder,device):
    model_dir = cfg.TRAIN.NET_G
    split_dir = 'valid'
    # Build and load the generator
    netG.load_state_dict(torch.load('models/%s/netG.pth'%(cfg.CONFIG_NAME), map_location=torch.device('cuda:0')))
    ...
    print(data_dic)
    for i in data_dic:  # (cfg.TEXT.CAPTIONS_PER_IMAGE)
        captions, cap_lens, keys = data_dic[i]
        batch_size=captions.shape[0]
        print(device)
        captions = Variable(torch.from_numpy(captions), volatile=True).to('cuda:0')
        cap_lens = Variable(torch.from_numpy(cap_lens), volatile=True).to('cuda:0')
        print(batch_size)
        cnt += batch_size
        # if step % 100 == 0:
        #     print('step: ', step)
        ...
        for j in range(batch_size):
            ...
            im = np.transpose(im, (1, 2, 0))
            im = Image.fromarray(im)
            fullpath = '%s_%s.png' % (s_tmp,input_text)
            im.save(fullpath)
...
if __name__ == "__main__":
    ...
    state_dict = torch.load(cfg.TEXT.DAMSM_NAME, map_location=torch.device('cuda:0'))
    ...
    if cfg.B_VALIDATION:
        if args.gen:
            gen_example(args.input_text,args.output_dir,dataset.wordtoix,netG,text_encoder,device)
        else:
